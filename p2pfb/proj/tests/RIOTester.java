...
public class RIOTester extends RIONode {
     ...
     * @throws IOException
     */
    public RIOTester() throws IOException {
        super();
     }
	...
	public void start() {
	    super.start();
		logOutput("Starting up... (id=" + instanceID + ")");
	...
	public void onRIOReceive(Integer from, int appProtocol, byte[] msg) {
		if (appProtocol != Protocol.RIOTEST_PKT) {
			logError("unknown protocol: " + appProtocol);
			return;
		}
	...
	public void failure(int from, int i) {
	 ...
	 public void onPeerFailure(int peer, Collection<RIOPacket> failedSends) {
	     System.out.println(this.addr + "received peer failure notification for " + peer);
	     receivedNums.put(peer, null);
	     nextNum.put(peer, 0);
     }
