...
public class Simulator extends Manager {
    ...
    private void executeEvents(ArrayList<Event> currentRoundEvents) {
        ...
        if (userControl == FailureLvl.EVERYTHING) {
            ...
            for (Event ev : currentRoundEvents) {
                if(ev.t == Event.EventType.DELIVERY) {
                    if(debug.PacketDisplayer.shouldDisplay(ev.p.getSrc(), ev.p.getDest(), ev.p.getProtocol(), ev.p.getPayload())) {
                        if(!debug.PacketDisplayer.useColors())
                            System.out.println(ev.toString());
                        else {
                            String color = debug.PacketDisplayer.getDisplayColor(ev.p.getProtocol());
                            main.AppUtil.printColored(ev.toString(), color);
                        }
                    }
                } else {
                    // else, just print it simply
                    System.out.println(ev.toString());
                }
                handleEvent(ev);
            }
